...
static const char g_bloom_gl_fs[] =
    ...
    "    {\n"
    "        float text_alpha = texture(u_texture, v_uv).w;\n"
    "        float sharpened_alpha = smoothstep(0.18, 0.82, text_alpha);\n"
    "        text_alpha = mix(text_alpha, sharpened_alpha, 0.35);\n"
    "        out_col.w *= text_alpha;\n"
    "    }\n"
    "    else if (v_elem_type == 2u)\n"
    "    {\n"
    "    }\n"
    ...
    "    }\n"
    "    else if (v_elem_type == 4u)\n"
    "    {\n"
    "        vec2 pa = v_frag_pos - v_center;\n"
    "        vec2 ba = v_half_size - v_center;\n"
    "        float h = clamp(dot(pa, ba) / dot(ba, ba), 0.0, 1.0);\n"
    "        float d = length(pa - ba * h) - v_corner_radii.x;\n"
    "        float aa = fwidth(d) * 0.75;\n"
    "        out_col.w *= 1.0 - smoothstep(-aa, aa, d);\n"
    "    }\n"
    "    else if (v_elem_type == 5u)\n"
    "    {\n"
    "        float d = sdTriangle(v_frag_pos, v_center, v_half_size, v_uv);\n"
    "        float aa = fwidth(d) * 0.75;\n"
    "        out_col.w *= 1.0 - smoothstep(-aa, aa, d);\n"
    "    }\n"
    "    else if (v_elem_type == 6u)\n"
    "    {\n"
    "        float d = sdRoundBox(v_frag_pos - v_center, v_half_size, v_corner_radii);\n"
    "        float aa = fwidth(d) * 0.75;\n"
    "        vec2 top_left = floor(v_center - v_half_size + 0.5);\n"
    "        vec2 local_pos = v_frag_pos - top_left;\n"
    "        vec2 grid = floor(local_pos / v_border_thickness);\n"
    "        float check = mod(grid.x + grid.y, 2.0);\n"
    "        out_col.w *= 1.0 - smoothstep(-aa, aa, d);\n"
    "        out_col.xyz *= mix(0.9, 1.1, check);\n"
    "    }\n"
    "    else if (v_elem_type == 7u)\n"
    "    {\n"
    "        float d = sdRoundBox(v_frag_pos - v_center, v_half_size, v_corner_radii);\n"
    "        float aa = fwidth(d) * 0.75;\n"
    "        vec2 uv = (v_frag_pos - (v_center - v_half_size)) / (v_half_size * 2.0);\n"
    "        vec3 hue_col = v_col.xyz;\n"
    "        vec3 sat_col = mix(vec3(1.0), hue_col, clamp(uv.x, 0.0, 1.0));\n"
    "        vec3 val_col = mix(vec3(0.0), sat_col, clamp(1.0 - uv.y, 0.0, 1.0));\n"
    "        out_col = vec4(val_col, v_col.w * (1.0 - smoothstep(-aa, aa, d)));\n"
    "    }\n"
    "    else if (v_elem_type == 8u)\n"
    "    {\n"
    "        float d = sdRoundBox(v_frag_pos - v_center, v_half_size, v_corner_radii);\n"
    "        float aa = fwidth(d) * 0.75;\n"
    "        vec2 uv = (v_frag_pos - (v_center - v_half_size)) / (v_half_size * 2.0);\n"
    "        vec3 hue_col = clamp(abs(mod(uv.x * 6.0 + vec3(0.0, 4.0, 2.0), 6.0) - 3.0) - 1.0, 0.0, 1.0);\n"
    "        out_col = vec4(hue_col, v_col.w * (1.0 - smoothstep(-aa, aa, d)));\n"
    "    }\n"
    "    else if (v_elem_type == 9u)\n"
    "    {\n"
    "        float d = sdRoundBox(v_frag_pos - v_center, v_half_size, v_corner_radii);\n"
    "        float aa = fwidth(d) * 0.75;\n"
    "        vec2 uv = (v_frag_pos - (v_center - v_half_size)) / (v_half_size * 2.0);\n"
    "        out_col = vec4(v_col.xyz, v_col.w * uv.x * (1.0 - smoothstep(-aa, aa, d)));\n"
    "    }\n"
    "    else\n"
    "    {\n"
