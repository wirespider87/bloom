...
static const char g_bloom_shader_src[] =
    ...
    "{\n"
    "    float2 s = float2(p.x > 0.0f ? r.y : r.x, p.x > 0.0f ? r.z : r.w);\n"
    "    float corner_r = p.y > 0.0f ? s.y : s.x;\n"
    ...
    "        float aa = fwidth(d) * 0.75f;\n"
    "        out_col.w *= 1.0f - smoothstep(-aa, aa, d);\n"
    "    }\n"
    ...
    "    }\n"
    "    else if (input.elem_type == 2)\n"
    "    {\n"
    "    }\n"
    "    else if (input.elem_type == 3)\n"
    "    {\n"
    "        float d = sdRoundBox(input.frag_pos - input.center, input.half_size, input.corner_radii);\n"
    "        float bd = abs(d) - input.border_thickness * 0.5f;\n"
    "        float aa = fwidth(bd) * 0.75f;\n"
    "        out_col.w *= 1.0f - smoothstep(-aa, aa, bd);\n"
    "    }\n"
    ...
    "        out_col.w *= 1.0f - smoothstep(-aa, aa, d);\n"
    "    }\n"
    "    else if (input.elem_type == 5)\n"
    "    {\n"
    "        float d = sdTriangle(input.frag_pos, input.center, input.half_size, input.uv);\n"
    "        float aa = fwidth(d) * 0.75f;\n"
    "        out_col.w *= 1.0f - smoothstep(-aa, aa, d);\n"
    "    }\n"
    "    else if (input.elem_type == 6)\n"
    "    {\n"
    "        float d = sdRoundBox(input.frag_pos - input.center, input.half_size, input.corner_radii);\n"
    "        float aa = fwidth(d) * 0.75f;\n"
    "        float2 top_left = floor(input.center - input.half_size + 0.5f);\n"
    "        float2 local_pos = input.frag_pos - top_left;\n"
    "        float2 grid = floor(local_pos / input.border_thickness);\n"
    "        float check = (grid.x + grid.y) % 2.0f;\n"
    "        out_col.w *= 1.0f - smoothstep(-aa, aa, d);\n"
    "        out_col.xyz *= lerp(0.9f, 1.1f, abs(check));\n"
    "    }\n"
    "    else if (input.elem_type == 7)\n"
    "    {\n"
    "        float d = sdRoundBox(input.frag_pos - input.center, input.half_size, input.corner_radii);\n"
    "        float aa = fwidth(d) * 0.75f;\n"
    "        float2 uv = (input.frag_pos - (input.center - input.half_size)) / (input.half_size * 2.0f);\n"
    "        float3 hue_col = input.col.xyz;\n"
    "        float3 sat_col = lerp(float3(1.0, 1.0, 1.0), hue_col, saturate(uv.x));\n"
    "        float3 val_col = lerp(float3(0.0, 0.0, 0.0), sat_col, saturate(1.0 - uv.y));\n"
    "        out_col = float4(val_col, input.col.w * (1.0f - smoothstep(-aa, aa, d)));\n"
    "    }\n"
    "    else if (input.elem_type == 8)\n"
    "    {\n"
    "        float d = sdRoundBox(input.frag_pos - input.center, input.half_size, input.corner_radii);\n"
    "        float aa = fwidth(d) * 0.75f;\n"
    "        float2 uv = (input.frag_pos - (input.center - input.half_size)) / (input.half_size * 2.0f);\n"
    "        float3 hue_col = saturate(abs(((uv.x * 6.0f + float3(0, 4, 2)) % 6.0f) - 3.0f) - 1.0f);\n"
    "        out_col = float4(hue_col, input.col.w * (1.0f - smoothstep(-aa, aa, d)));\n"
    "    }\n"
    "    else if (input.elem_type == 9)\n"
    "    {\n"
    "        float d = sdRoundBox(input.frag_pos - input.center, input.half_size, input.corner_radii);\n"
    "        float aa = fwidth(d) * 0.75f;\n"
    "        float2 uv = (input.frag_pos - (input.center - input.half_size)) / (input.half_size * 2.0f);\n"
    "        out_col = float4(input.col.xyz, input.col.w * saturate(uv.x) * (1.0f - smoothstep(-aa, aa, d)));\n"
    "    }\n"
    "    else\n"
    "    {\n"
